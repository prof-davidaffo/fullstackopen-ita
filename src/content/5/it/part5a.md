...
Al momento il frontend mostra le note e permette di modificarne lo stato da importante a non importante e viceversa. Le modifiche al backend della parte 4 impediscono però di aggiungerne di nuove: ora insieme alla nota è richiesto un token che verifichi l'identità dell'utente.
...
Senza connessione al backend il frontend non mostra alcuna nota. Avvia il backend della Parte 4 con _npm run dev_: sarà disponibile sulla porta 3001. Lascialo attivo e, in un altro terminale, avvia il frontend con _npm run dev_. Vedrai così le note salvate nel database MongoDB.
