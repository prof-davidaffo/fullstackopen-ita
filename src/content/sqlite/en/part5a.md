...
### Connecting to the SQLite backend

Use the backend completed in Part 4, not the MongoDB backends linked in the original examples. The frontend uses the same JSON API.

The SQLite backend also protects PUT and DELETE. Send the token on every write; the services/notes.js example below includes these changes.

Call noteService.setToken(null) on logout. Apply the same pattern to the blog service. Assign an author to any old notes as explained in 4c before starting Part 5.
...
At the moment the frontend shows existing notes. Changing importance now also requires a valid token. New notes cannot be added anymore because of the changes made to the backend in part 4: the backend now expects that a token verifying a user's identity is sent with the new note.
