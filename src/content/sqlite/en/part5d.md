...
**SQLite track:** use the Part 4 test-env.cjs file, setting NODE_ENV=test and a test-only SECRET. The E2E backend uses an in-memory database separate from development. Do not use --watch: restarts would erase test state. For the blog app replace DELETE FROM notes with DELETE FROM blogs, deleting blogs before users. Never mount the reset router outside test mode.
