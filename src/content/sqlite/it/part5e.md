...
**Percorso SQLite:** continua con il backend locale della Parte 4 e il servizio Axios autenticato della 5a. I repository esterni mostrano la versione originale: non sostituire il tuo backend con quelli degli esempi. Dopo gli esercizi trovi il capitolo finale sulla pubblicazione.
...
Gli identificatori del backend SQLite sono UUID stringa: confrontali direttamente con il parametro dell’URL, senza Number(id).
...
const Note = ({ note, toggleImportanceOf, deleteNote }) => {  // highlight-line
  const id = useParams().id
